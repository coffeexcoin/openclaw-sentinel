...
const conditionSchema = z.object({
  path: z.string().min(1),
  op: z.enum(['eq', 'neq', 'gt', 'gte', 'lt', 'lte', 'exists', 'absent', 'contains', 'matches', 'changed']),
...
function scanNoCodeLike(input: unknown, parentKey = ''): void {
  ...
  if (typeof input === 'string') {
    if (codeyValuePattern.test(input)) throw new Error(`Code-like value rejected at ${parentKey || '<root>'}`);
    return;
  }
  ...
  if (typeof input === 'object') {
    for (const [key, value] of Object.entries(input as Record<string, unknown>)) {
      if (codeyKeyPattern.test(key)) throw new Error(`Code-like field rejected: ${parentKey ? `${parentKey}.` : ''}${key}`);
